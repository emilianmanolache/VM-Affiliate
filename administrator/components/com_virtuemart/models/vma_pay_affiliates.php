...
class VirtueMartModelVma_pay_affiliates extends VmModel {
	...
	function _buildQuery($total = false) {
		...
		$condition	= " WHERE `commissions` >= '" . $vmaSettings->pay_balance . "' AND `method` != '' AND `method` != 'N/A' AND `blocked` != '1'";
		
		$condition .= $search ? " AND (" . $vmaHelper->prepareSearch($searchIn, $search) . ") " : NULL;
