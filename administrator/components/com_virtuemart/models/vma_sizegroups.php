...
class VirtueMartModelVma_sizegroups extends VmModel {
	...
	function _buildQuery($total = false) {
		...
		$condition	.= $search ? $vmaHelper->prepareSearch($searchIn, $search) : NULL;
		
		// build query
			
		$query		= !$total ? 
			
				 	  "SELECT * FROM #__vm_affiliate_size_groups WHERE " 				. $condition . " ORDER BY `width` ASC":
				 
					  "SELECT COUNT(*) FROM #__vm_affiliate_size_groups WHERE " 		. $condition;
