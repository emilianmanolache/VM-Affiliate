...
class VirtueMartModelVma_banners extends VmModel {
	...
	function _buildQuery($total = false) {
		...
		$condition	= $search ? " WHERE " . $vmaHelper->prepareSearch($searchIn, $search, "banners") : NULL;
		...
		$query		= !$total ? 
				 	  ...
				 	  "SELECT banners.*, sizegroups.`name` AS sizegroup FROM #__vm_affiliate_banners banners " 	. $join . $condition :
				 
					  "SELECT COUNT(*) FROM #__vm_affiliate_banners banners " 									. $join	. $condition;
