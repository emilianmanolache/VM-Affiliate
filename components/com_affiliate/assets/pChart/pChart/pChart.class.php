...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
   ...
   function drawScale(&$Data,&$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1, $reporttype)
    {
         ...
         foreach ( $Data as $Key => $Values )
          {
           ...
           foreach ( $DataDescription["Values"] as $Key2 => $ColName )
            {
             if (isset($Data[$Key2][$ColName]))
              {
               $Value = $Data[$Key2][$ColName];
               if ( is_numeric($Value) )
                $Sum  += $Value;
              }
            }
     ...
     foreach ( $Data as $Key => $Values )
      {
       if ( $ID % $SkipLabels == 0 )
		...
		if ($reporttype == "year") {
			...
			$month = array(JText::_("JANUARY_SHORT"), JText::_("FEBRUARY_SHORT"), JText::_("MARCH_SHORT"), JText::_("APRIL_SHORT"), 
         ...
         $Position   = imageftbbox($this->FontSize,$Angle,$this->FontName,$dasyum);
         ...
         if ( $Angle == 0 )
          {
           ...
           imagettftext($this->Picture,$this->FontSize,$Angle,floor($XPos)-floor($TextWidth/2),$YPos,$C_TextColor,$this->FontName,$dasyum);
          }
         else
          {
